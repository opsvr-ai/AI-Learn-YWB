...
| `claude.html` | Claude Code：Node.js安装(前置条件)、安装配置、开发模式详解(Plan/Auto/讲解/学习)、子代理系统、常用命令、质量保障、延伸阅读 |
...
├── claude-code-definitive.pdf
├── claude-code-skills.pdf
└── claude-code-explore.pdf
素材/                       # 原始素材（临时存放，不直接引用）
```
...
| `claude.html` | 第 382 行（说明文字），第 421 行（配置表） |
| `packages/settings.json` | `ANTHROPIC_BASE_URL` 字段 |
| `素材/claude-code/settings.json` | `ANTHROPIC_BASE_URL` 字段 |
